...
const disabledStyles =
  "cursor-not-allowed opacity-50 bg-gray-200 text-gray-400 border-none";
...
const PdvButton = (props: TPdvButton) => {
  ...
  const selectedTextColor = () => {
    if (variant === "contained")
      return textColor ? `var(--${String(textColor)})` : "var(--white)";
    return textColor
      ? `var(--${String(textColor)})`
      : `var(--${String(props.color)})`;
  };
  ...
  const style = () => {
    return {
      ...
      border:
        variant === "outlined" ? `1px solid var(--${String(color)})` : "none",
      boxShadow:
    ...
    <span className={`${props?.disabled ? "cursor-not-allowed" : ""}`}>
      <Button
        className={`${className ?? ""} ${structure} ${
          disabled ? disabledStyles : "hover:opacity-70"
        }`}
        {...rest}
        sx={sx}
        style={{ ...style() }}
